...
namespace thelostgrimoire
{
    ...
    internal static class Helpers
    {
        ...
        public static void Load()
        {
            ...
            foreach (var blueprint in Main.library.GetAllBlueprints())
            {
                ...
                var unit = blueprint as BlueprintUnit;
                if (unit != null)
                {
                    ennemies.Add(unit);
                    continue;
                }
                var weapontype = blueprint as BlueprintWeaponType;
                if (weapontype != null)
                {
        ...
        //@@Nemorga : added helper for armor proficiency requisite
        public static PrerequisiteProficiency PrerequisiteArmorProficiency(this ArmorProficiencyGroup[] armor, bool any = false)
        {
            var result = Create<PrerequisiteProficiency>();
            result.ArmorProficiencies = armor;
            result.Group = any ? Prerequisite.GroupType.Any : Prerequisite.GroupType.All;
            return result;
        }
    ...
    /*   public static SelectionEntry CreateSelectionEntryComponent(bool Parametrized, String Selectionguid, params BlueprintFeature[] Features )
        {

            SelectionEntry Entry = null;
            var library = Main.library;
            Entry.IsParametrizedFeature = Parametrized;
            Entry.
            Entry.Selection = library.Get<BlueprintFeatureSelection>(Selectionguid);
        ...
        {
            var md5 = MD5.Create();
            byte[] data = md5.ComputeHash(Encoding.Default.GetBytes("cc9230bb4d714b0aa5c8cab6e3eca4f6" +s));
